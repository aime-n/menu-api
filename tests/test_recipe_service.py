...
def test_create_recipe_and_links(session):
    ...
    recipe = recipe_service.create_recipe(
        session, name="Salad", instructions="Mix all.", ingredients_data=ingredients_data
    )
    assert recipe.name == "Salad"
    # Check links
    links = session.exec(
        RecipeIngredientLink.__table__.select().where(RecipeIngredientLink.recipe_id == recipe.id)
    ).all()
    assert len(links) == 2
...
def test_delete_recipe_by_id(session):
    recipe = recipe_service.create_recipe(session, "Pie", "Bake", [("Apple", "2", "pcs")])
